...
#Inicio do Programa
    ...
    def buscarProdutos():
        global num_tela, contador,designerTabela
        print("\n")
        print("_"*55)
    ...
    def cadastroProdutos():
        ...
        texto_cifrado = '' # string para armazenar o texto criptografado
        vetorNome = [] # transforma a palavra em um vetor
        matriz = [] # variavel para armazenar os pares de letras
        novalinha = [] # variavel para armazenar os indices ímpares
        linha = [] # variavel para armazenar os índices pares
        chave = [[4,3],[1,2]]
        cifrado = [] # matriz para guardar os indices criptografados
        vetorTransposto = [] # vetor transposto
        trans = [] # matriz transposta

        #verificar se tamanho da string é impar
        if(len(descricao_cadastro)%2 != 0):
            descricao_cadastro = descricao_cadastro + descricao_cadastro[len(descricao_cadastro)-1] #acrescenta a ultima letra se frase for impar
        #transformar a string em vetor
        for i in range(len(descricao_cadastro)):
            posicoes = alfabeto.find(descricao_cadastro[i])
            #print(posicoes)
            vetorNome.append(posicoes)
        #print(vetorNome)
        # transformar os pares dos vetores em matriz
        for x in range(0,len(descricao_cadastro)):
            par = x%2
            if par == 0:
                linha.append(vetorNome[x]) #vetor da primeira linha da tabela
            else:
                novalinha.append(vetorNome[x]) #vetor da segunda linha da tabela
        ...
        for linha in range(2):#cria nova linha
            cifrado.append([])
            for coluna in range(colunas):#cria nova coluna
                cifrado[linha].append(0)
                for valores in range(2):# preenche a matriz com o produto das matrizes
                    cifrado[linha][coluna] +=chave[linha][valores]*matriz[valores][coluna]
        #laço para acessar os valores da matriz 
        for linha in range(2):
            for coluna in range(len(cifrado[0])):
                if cifrado[linha][coluna] > 26:# caso nº for maior que 26 
                    cifrado[linha][coluna] = (cifrado[linha][coluna]%26)# recebe modulo de 26

        for l in range(len(cifrado[0])):#preenche o vetor transposto com os valores da matriz
            for c in range(2):
                vetorTransposto = cifrado[c][l] #pega cada elemento da matriz obs: oq era coluna virou linha
                trans.append(vetorTransposto)
